...
pg_line_as_points(pgLineObject *self, PyObject *arg)
{
    ...
    point = pg_tuple_couple_from_values_double(line->xa, line->ya);
    if (!point) {
        Py_DECREF(list);
        return NULL;
    }
    PyList_SET_ITEM(list, 0, point);
    point = pg_tuple_couple_from_values_double(line->xb, line->yb);
    if (!point) {
        Py_DECREF(list);
        return NULL;
    }
    ...
    if (!N) {
        ...
        point = pg_tuple_couple_from_values_double((line->xa + line->xb) / 2,
                                       (line->ya + line->yb) / 2);
        if (!point) {
            Py_DECREF(list);
            return NULL;
    ...
    Py_ssize_t i;
    for (i = 1; i < N + 1; i++) {
        point = pg_tuple_couple_from_values_double(x, y);
        if (!point) {
            Py_DECREF(list);
            return NULL;
        }
...
static PyObject *
pg_line_getlength_sqr(pgLineObject *self, void *closure)
{
    return PyFloat_FromDouble(pgLine_LengthSquared(&self->line));
}
...
{
    return pg_tuple_couple_from_values_double((self->line.xa + self->line.xb) / 2,
...
static PyGetSetDef pg_line_getsets[] = {
    ...
    {"length", (getter)pg_line_getlength_sqr, NULL, NULL, NULL}
